...
void HoymilesRadio_NRF::init(SPIClass* initialisedSpiBus, uint8_t pinCE, uint8_t pinSS, uint8_t pinIRQ)
{
    _dtuSerial.u64 = 0;

    Hoymiles.getMessageOutput()->println("NRF: Starting init!");
    _spiPtr.reset(initialisedSpiBus);
    _radio.reset(new RF24(pinCE, pinSS));
...
IRAM_ATTR void HoymilesRadio_NRF::handleIntr()
{
...
void HoymilesRadio_NRF::sendEsbPacket(CommandAbstract* cmd)
{
    ...
    Hoymiles.getMessageOutput()->printf("TX %s Channel: %d --> ", cmd->getCommandName().c_str(), _radio->getChannel());
